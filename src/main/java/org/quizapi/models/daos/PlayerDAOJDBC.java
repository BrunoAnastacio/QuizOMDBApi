...
import org.quizapi.util.DBManager;
...
public class PlayerDAOJDBC {
    ...
    public void insert(@NotNull Player player) {
        ...
        try {
            ...
            ps.setInt(1, player.getId().intValue());
    ...
    public List<String> toList() throws NotFoundIDException {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                Player player = new Player((long)id, name, score, timestampSubscription, timestampLastUpdate);
   ...
   public Player update(int id, Player toUpdate) throws NotFoundIDException {
    ...
    public Player selectById(int id) {
        ...
        try {
            ...
            int id_response = resultSet.getInt(1);
            String name = resultSet.getString(2);
            int score = resultSet.getInt(3);
            Timestamp timestampSubscription = resultSet.getTimestamp(4);
            Timestamp timestampLastUpdate = resultSet.getTimestamp(5);

            player = new Player((long)id_response, name, score, timestampSubscription, timestampLastUpdate);
